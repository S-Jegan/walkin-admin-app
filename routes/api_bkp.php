...
// })->middleware('auth:api');
	...
	*/
	 Route::post('interviews/search', function (Request $request) {
		...
		->whereRaw('interview_end_date >= CURDATE()')
	...
	*/
	...
	->where('is_active',1)->whereRaw('interview_end_date >= CURDATE()')
	...
	*/
	Route::get('keywords', function () {
		$response = WalkinInterviewDetails::select('skills','designation')->where('is_active',1)->get();
		return $response;
	});
	...
	*/
	Route::post('interviews/today', function (Request $request) {
		$response = WalkinInterviewDetails::where('is_active',1)
		->whereRaw('interview_end_date >= CURDATE()')
		->whereRaw('CURDATE() BETWEEN INTERVIEW_START_DATE AND INTERVIEW_END_DATE')
		->with(['company','industry:id,name'])->orderBy('interview_start_date','ASC')
		->paginate($request->per_page,['*'], 'page', $request->page);
 		return $response;
	  });
	...
	*/
	Route::post('interviews/tomorrow', function (Request $request) {
		$response = WalkinInterviewDetails::where('is_active',1)
		->whereRaw('interview_end_date >= CURDATE()+1')
		->whereRaw('CURDATE()+1 BETWEEN INTERVIEW_START_DATE AND INTERVIEW_END_DATE')
		->with(['company','industry:id,name'])->orderBy('interview_start_date','ASC')
		->paginate($request->per_page,['*'], 'page', $request->page);
 		return $response;
	});
	...
	*/
	Route::post('interviews/freshers', function (Request $request) {
		$response = WalkinInterviewDetails::where('is_active',1)
		->whereRaw('interview_end_date >= CURDATE() and (audience in ("Freshers","Both")  or exp_start=0 or category_id=14)')
		->with(['company','industry:id,name'])
		->paginate($request->per_page,['*'], 'page', $request->page);
		return $response;
	});
